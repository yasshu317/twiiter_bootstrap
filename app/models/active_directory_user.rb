...
class ActiveDirectoryUser
  ### BEGIN CONFIGURATION ###
  SERVER = 'zeus.corp.ads.valuelabs.net'   # Active Directory server name or IP
  PORT = 389                    # Active Directory server port (default 389)
  BASE = 'DC=corp,DC=ads,DC=valuelabs,DC=net'    # Base to search from
  DOMAIN = 'corp.ads.valuelabs.net'        # For simplified user@domain format login
  ...
  def self.authenticate(login, pass)
    debug_log login.empty? or pass.empty?
    return nil if login.empty? or pass.empty?
    conn = Net::LDAP.new  :host => "zeus.corp.ads.valuelabs.net",
                      :port => "389",
